...
const pairingQR = document.querySelector<HTMLCanvasElement>("#pairing-qr")!;
const pairingDialogTitle = document.querySelector<HTMLElement>("#pairing-dialog-title")!;
const pairingCodeField = document.querySelector<HTMLElement>("#pairing-code-field")!;
...
const pairingListenValue = document.querySelector<HTMLElement>("#pairing-listen-value")!;
const pairingTunnelValue = document.querySelector<HTMLElement>("#pairing-tunnel-value")!;
const copyPairingButton = document.querySelector<HTMLButtonElement>("#copy-pairing")!;
const copyPairingListenButton = document.querySelector<HTMLButtonElement>("#copy-pairing-listen")!;
const copyPairingTunnelButton = document.querySelector<HTMLButtonElement>("#copy-pairing-tunnel")!;
...
let pairingDialogMode: "pairing" | "connection" | "" = "";
let controlAddr = "";
let controlAddrs: string[] = [];
let serverUUID = "";
let toastTimer = 0;
...
async function updatePairingDialog() {
    if (!pairingDialogMode) return;
    const pairing = pairingDialogMode === "pairing";
    pairingDialogTitle.textContent = t(pairing ? "pairDevice" : "connectDevice");
    pairingCodeField.hidden = !pairing;
    pairingExpiry.hidden = !pairing;
    pairingValue.textContent = pairingToken;
    pairingExpiry.textContent = t("expires", {value: pairingExpiresAt});
    pairingListenValue.textContent = controlAddrs.join(", ") || "-";
    pairingTunnelValue.textContent = tunnelURL || "-";
    ...
    await QRCode.toCanvas(pairingQR, JSON.stringify({
        ...
        listenAddresses: controlAddrs,
        tunnelAddress: tunnelURL,
        ...(pairing ? {pairingCode: pairingToken} : {}),
    }), {width: 240, margin: 1, color: {dark: "#101218", light: "#ffffff"}});
}
...
async function refresh() {
    ...
    try {
        ...
        connectButton.disabled = !serverUUID;
...
async function refreshDevices() {
    try {
        ...
        if (pairingToken && !pairingActive) hidePairingDialog();
    } catch (error) {
...
async function bindDevice() {
    ...
    try {
        ...
        pairingDialogMode = "pairing";
        pairingDialog.showModal();
        await updatePairingDialog();
    } catch (error) {
...
async function startPairingAppServer() {
    startPairingAppServerButton.disabled = true;
    try {
        renderAppServer(await AppService.StartAppServer());
    } catch (error) {
        showToast(error instanceof Error ? error.message : t("unableChangeAppServer"), true);
        renderAppServer(await AppService.AppServerState());
    }
...
async function startPairingTunnel() {
    startPairingTunnelButton.disabled = true;
    try {
        renderTunnel(await AppService.StartTunnel());
    } catch (error) {
        showToast(error instanceof Error ? error.message : t("unableChangeTunnel"), true);
        renderTunnel(await AppService.TunnelState());
    }
...
copyPairingTunnelButton.addEventListener("click", () => void copyText(tunnelURL, t("tunnelAddress")));
